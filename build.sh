...
send_telegram_message() {
  ...
  if [ -z "$TELEGRAM_BOT_TOKEN" ] || [ -z "$TELEGRAM_CHAT_ID" ]; then
    echo "!! TELEGRAM_BOT_TOKEN / TELEGRAM_CHAT_ID belum diset, lewati kirim pesan Telegram."
    return 0
  fi
...
# ============================================================
# Fungsi upload file (dokumen) ke Telegram (via bot token & chat id/grup)
# ============================================================
upload_to_telegram() {
  ...
  if [ -z "$TELEGRAM_BOT_TOKEN" ] || [ -z "$TELEGRAM_CHAT_ID" ]; then
    echo "!! TELEGRAM_BOT_TOKEN / TELEGRAM_CHAT_ID belum diset, lewati upload ke Telegram."
    return 0
  fi

  if [ ! -f "$FILE_TO_UPLOAD" ]; then
    echo "!! File '${FILE_TO_UPLOAD}' tidak ditemukan, upload ke Telegram dibatalkan."
    return 1
  fi

  echo "==> Mengupload ${FILE_TO_UPLOAD} ke Telegram..."

  curl -s -X POST "https://api.telegram.org/bot${TELEGRAM_BOT_TOKEN}/sendDocument" \
    -F chat_id="${TELEGRAM_CHAT_ID}" \
    -F caption="${CAPTION}" \
    -F document=@"${FILE_TO_UPLOAD}" >/dev/null
}
...
on_build_error() {
  ...
  echo "==> Mengirim notifikasi build gagal ke Telegram..."
  send_telegram_message "❌ *Build GAGAL* untuk kernel di \`$KERNEL_DIR\`. Error log sudah diupload ke Gofile."
  upload_to_telegram "$LOG_FILE" "Error log build gagal"
  exit 1
}
...
# (upload error log ke Gofile & Telegram) lalu script berhenti.
trap on_build_error ERR

echo "==> Mengirim notifikasi mulai build ke Telegram..."
send_telegram_message "🛠️ Mulai build kernel di \`$KERNEL_DIR\` (${BUILD_TIME})..."
...
# ===== 📨 Kirim notifikasi build sukses & upload zip ke Telegram =====
echo "==> Mengirim notifikasi build sukses ke Telegram..."
send_telegram_message "✅ *Build SUKSES!* \`${ZIP_NAME}\` siap diflash."
upload_to_telegram "$KERNEL_DIR/${ZIP_NAME}" "${ZIP_NAME}"
